...
**PROVISIONAL.** Single fold (fold 0), single seed (42). The spread across all five architectures
is smaller than any spread estimate available, so nothing here supports a ranking. These will be
regenerated once the full 5-fold cross-validation completes.
...
The single most important element of the comparison figure is the **dashed red line at 0.187** —
a metadata-only baseline that decodes no pixels at all. A model below it has learned nothing from
the images. All five sit at 0.49–0.52, roughly 2.6× the bar, which is what licenses treating any
of these as a real result.

The single most important thing the figure does *not* show is a winner. A 0.028 spread across a
7× parameter range and three model families is not a ranking; it is noise until the remaining
folds land.

Generated by `scripts/make_staging_graphs.py` in the development repository.
